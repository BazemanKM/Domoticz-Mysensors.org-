...
Script = 'script_time_wasmachinebijkeuken'
Debug = 'Y'
-- Functions
...
-- Variable
local washer_status_uservar = 'washingmachineb_status'
local energy_consumption = 'Wasmachine bijkeuken' --Name of Z-Wave plug that contains actual consumption of washingmachine (in Watts)
local washer_counter_uservar = 'washingmachineb_counter' --Name of the uservariable that will contain the counter that is needed
...
sWatt, sTotalkWh = otherdevices_svalues['Wasmachine bijkeuken']:match("([^;]+);([^;]+)")
...
commandArray['SendNotification']='De wasmachine bijkeuken is klaar'
